...
screenshot_alt: "@sebastienrousseau/size-limit-config architecture and rule enforcement."
footer_note: "@sebastienrousseau/size-limit-config is part of the @sebastienrousseau/config suite. Published under Apache-2.0 OR MIT."
copyright: "© 2026 Sebastien Rousseau. Licensed under Apache-2.0 OR MIT."
translation_key: "home"
title: "@sebastienrousseau/size-limit-config — Shareable Size Limit configuration rules for continuous JavaScript bundle size budget enforcement."
description: "Shareable Size Limit configuration rules for continuous JavaScript bundle size budget enforcement."
keywords: "size-limit-config, size-limit, bundle size, performance budget, webpack, esbuild, rollup, developer tools, sebastien rousseau, standard"
hero_tag: "v0.0.3"
hero_tagline: "Continuous JavaScript bundle size and execution budget enforcement"
headline: "@sebastienrousseau/size-limit-config"
lead: "Shareable Size Limit configuration rules for continuous JavaScript bundle size budget enforcement."
cta_primary: "Install @sebastienrousseau/size-limit-config"
...
snippet_caption: "Install @sebastienrousseau/size-limit-config"
snippet_code: "npm install --save-dev @sebastienrousseau/size-limit-config"
...
## What is @sebastienrousseau/size-limit-config?

**@sebastienrousseau/size-limit-config** is an enterprise-grade, shareable configuration for [Size Limit](https://github.com/sebastienrousseau/size-limit-config)—the performance budget tool calculating bundle size and running time in continuous integration. Designed to deliver vetted, deterministic defaults, this package eliminates manual setup boilerplate and establishes rigorous code quality standards across modern software development pipelines.

By adopting this configuration, engineering teams ensure unified consistency across local development, continuous integration, and automated deployments. The package adheres strictly to the OpenSSF Best Practices badge program (ID: 14519) and achieves top-tier security ratings via Scorecard audits.
...
## Why Developers Choose @sebastienrousseau/size-limit-config

Modern JavaScript and TypeScript repositories frequently suffer from configuration drift, fragmented standards, and maintenance overhead. **@sebastienrousseau/size-limit-config** directly solves these challenges:
...
Integrating `@sebastienrousseau/size-limit-config` into any new or existing repository requires only three simple steps.
...
### Step 2: Configure `.size-limit.json`

Create or update your `.size-limit.json` file at the root of your project to extend the shared configuration:
...
### Step 3: Run Size Limit in Your Workflow

Execute Size Limit directly or configure standard quality scripts in your `package.json`:

```bash
npx size-limit
```

Calculate bundle byte size, gzip overhead, and client execution latency against budgets.
...
### Why is Size Limit essential in CI?
Size Limit prevents accidental performance regressions by breaking CI builds if a pull request introduces oversized dependencies or bundle inflation.

### Can I override individual rules in my project?
Yes. Every configuration allows local overrides directly within your `.size-limit.json` file or inline settings. Detailed override recipes are documented in the [Usage Guide](usage/).

### Is @sebastienrousseau/size-limit-config safe for production CI/CD?
