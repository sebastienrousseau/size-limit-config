...
%global npm_bare      size-limit-config
%global sitelib       %{_prefix}/lib/node_modules

Name:           nodejs-sebastienrousseau-size-limit-config
Version:        0.0.7
Release:        1%{?dist}
Summary:        Shareable Size Limit configuration rules enforcing strict performance bundle budgets

License:        Apache-2.0 OR MIT
URL:            https://github.com/sebastienrousseau/size-limit-config
Source0:        %{name}-%{version}.tar.gz
BuildArch:      noarch
Requires:       nodejs >= 20

%description
@sebastienrousseau/size-limit-config is a shareable configuration preset with no runtime
...
install -pm 0644 .size-limit.js %{buildroot}%{sitelib}/%{npm_scope}/%{npm_bare}/
