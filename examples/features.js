...
console.log("=== 100% Feature Showcase: @sebastienrousseau/size-limit-config ===");
assert(Array.isArray(config) && config.length === 2);
assert(config.some(item => item.path === "dist/index.js" && item.limit === "10 KB"));
assert(config.some(item => item.path === "dist/index.mjs" && item.limit === "10 KB"));
